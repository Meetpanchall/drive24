...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    q('UPDATE test_drives t JOIN listings l ON l.id = t.listing_id SET t.status = ? WHERE t.id = ? AND l.seller_id = ?', [$status, $id, $u['id']]);
...
<div class="table-wrap"><table class="data">
  <thead><tr><th>Buyer</th><th>Car</th><th>Mode</th><th>Slot</th><th>Address</th><th>Status</th><th>Update</th></tr></thead>
  <tbody>
  <?php if (!$rows): ?><tr><td colspan="7" class="empty">No test drive requests.</td></tr><?php endif; ?>
  <?php foreach ($rows as $r): ?>
    <tr><td><b><?= e($r['buyer']) ?></b><div class="muted num" style="font-size:12px"><?= e((string) $r['mobile']) ?></div></td>
      ...
      <td><form method="post" style="display:flex;gap:6px"><?= csrfField() ?><input type="hidden" name="td_id" value="<?= (int) $r['id'] ?>">
        <select class="form-select" name="status" style="width:130px"><option value="confirmed">Confirm</option><option value="completed">Complete</option><option value="cancelled">Cancel</option></select>
